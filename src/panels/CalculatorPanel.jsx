// src/panels/CalculatorPanel.jsx
...
const CalculatorPanel = () => {
    ...
    const safeEval = (expr) => {
        ...
        try {
            const openBrackets = (jsExpr.match(/\(/g) || []).length;
            const closeBrackets = (jsExpr.match(/\)/g) || []).length;
            if (openBrackets !== closeBrackets) {
                return null;
            }
    ...
    const handleParentheses = () => {
        ...
        if (readyForNewInput) {
            ...
            setExpression(expression + '(');
        } else if (openCount > closeCount && !/[+−×÷(]$/.test(expression)) {
            setExpression(expression + ')');
        } else {
            setExpression(expression + '×(');
        }
    ...
    const buttonLayout = [
        [
            ...
            { label: '( )', type: 'function', action: handleParentheses },
